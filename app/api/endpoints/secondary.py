...
@router.get("/ipr_status",
            response_model=list[IprStatusDoc],
            response_model_exclude_none=True)
async def list_ipr_statuses(
...
@router.get("/ipr_competency",
            response_model=list[CompetencyDoc],
            response_model_exclude_none=True)
async def list_ipr_competencies(
    session: AsyncSession = Depends(get_async_session)
):
...
@router.get("/task_status",
            response_model=list[TaskStatusDoc],
            response_model_exclude_none=True)
...
@router.get("/specialty",
            response_model=list[SpecialtyDoc],
            response_model_exclude_none=True)
...
@router.get("/education",
            response_model=list[EducationDoc],
            response_model_exclude_none=True)
